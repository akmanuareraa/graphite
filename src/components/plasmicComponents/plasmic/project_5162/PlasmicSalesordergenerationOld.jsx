// @ts-nocheck
/* eslint-disable */
/* tslint:disable */
/* prettier-ignore-start */
/** @jsxRuntime classic */
/** @jsx createPlasmicElementProxy */
/** @jsxFrag React.Fragment */
// This class is auto-generated by Plasmic; please do not edit!
// Plasmic Project: 5GT6nVB5WgvLWb2iBXTk1E
// Component: J0RfGE8oivh
import * as React from "react";
import * as p from "@plasmicapp/react-web";
import {
  hasVariant,
  classNames,
  createPlasmicElementProxy,
  deriveRenderOpts
} from "@plasmicapp/react-web";
import ButtonB from "../../ButtonB"; // plasmic-import: oSXCQ8ez26/component
import "@plasmicapp/react-web/lib/plasmic.css";
import projectcss from "./plasmic_project_5162.module.css"; // plasmic-import: 5GT6nVB5WgvLWb2iBXTk1E/projectcss
import sty from "./PlasmicSalesordergenerationOld.module.css"; // plasmic-import: J0RfGE8oivh/css

export const PlasmicSalesordergenerationOld__VariantProps = new Array("test");

export const PlasmicSalesordergenerationOld__ArgProps = new Array();

function PlasmicSalesordergenerationOld__RenderFunc(props) {
  const { variants, args, overrides, forNode } = props;
  return (
    <div
      data-plasmic-name={"root"}
      data-plasmic-override={overrides.root}
      data-plasmic-root={true}
      data-plasmic-for-node={forNode}
      className={classNames(
        projectcss.all,
        projectcss.root_reset,
        projectcss.plasmic_default_styles,
        projectcss.plasmic_tokens,
        sty.root
      )}
    >
      <div
        className={classNames(
          projectcss.all,
          projectcss.__wab_text,
          sty.text__u8G8X
        )}
      >
        {"SALES ORDER GENERATION"}
      </div>

      <div
        data-plasmic-name={"applicationForm"}
        data-plasmic-override={overrides.applicationForm}
        className={classNames(projectcss.all, sty.applicationForm, {
          [sty.applicationFormtest]: hasVariant(variants, "test", "test")
        })}
      >
        <p.Stack
          as={"div"}
          hasGap={true}
          className={classNames(projectcss.all, sty.freeBox__efEAm, {
            [sty.freeBoxtest__efEAmxuUp0]: hasVariant(variants, "test", "test")
          })}
        >
          <div
            className={classNames(projectcss.all, sty.freeBox__cLct6, {
              [sty.freeBoxtest__cLct6XuUp0]: hasVariant(
                variants,
                "test",
                "test"
              )
            })}
          >
            <div
              className={classNames(projectcss.all, sty.freeBox__d2Cvq, {
                [sty.freeBoxtest__d2CvqxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__yXq2N,
                  {
                    [sty.texttest__yXq2NxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {hasVariant(variants, "test", "test") ? "PO No" : "Order ID"}
              </div>

              {true ? (
                <div className={classNames(projectcss.all, sty.freeBox__d13G1)}>
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__r8WyG
                    )}
                  >
                    {"#"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__t4Knu,
                      {
                        [sty.texttest__t4KnUxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"ABC123"}
                  </div>
                </div>
              ) : null}
            </div>
          </div>

          {(hasVariant(variants, "test", "test") ? true : false) ? (
            <p.Stack
              as={"div"}
              hasGap={true}
              className={classNames(projectcss.all, sty.freeBox__xiabc, {
                [sty.freeBoxtest__xiabcxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              {(hasVariant(variants, "test", "test") ? true : true) ? (
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__aAuIc,
                    {
                      [sty.texttest__aAuICxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Order To: "}
                </div>
              ) : null}
              {(hasVariant(variants, "test", "test") ? true : true) ? (
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__fgI9C,
                    {
                      [sty.texttest__fgI9CxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Ship To:"}
                </div>
              ) : null}
            </p.Stack>
          ) : null}
          {(hasVariant(variants, "test", "test") ? true : true) ? (
            <div
              className={classNames(projectcss.all, sty.freeBox__zsjiV, {
                [sty.freeBoxtest__zsjiVxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(projectcss.all, sty.freeBox__kEy8W, {
                  [sty.freeBoxtest__kEy8WxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__j96Pf,
                    {
                      [sty.texttest__j96PFxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__o5T9,
                    {
                      [sty.texttest__o5T9XuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__d2BpO,
                    {
                      [sty.texttest__d2BpOxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>

              <div
                className={classNames(projectcss.all, sty.freeBox__qe1I9, {
                  [sty.freeBoxtest__qe1I9XuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text___5BxCx,
                    {
                      [sty.texttest___5BxCXxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__oIamV,
                    {
                      [sty.texttest__oIamVxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text___8VOie,
                    {
                      [sty.texttest___8VOiExuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>

              <div
                className={classNames(projectcss.all, sty.freeBox__bZhtu, {
                  [sty.freeBoxtest__bZhtUxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__f5El,
                    {
                      [sty.texttest__f5ElXuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__aApF2,
                    {
                      [sty.texttest__aApF2XuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__rDiFz,
                    {
                      [sty.texttest__rDiFzxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>

              <div
                className={classNames(projectcss.all, sty.freeBox__aYvGx, {
                  [sty.freeBoxtest__aYvGXxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__pj1Yk,
                    {
                      [sty.texttest__pj1YKxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text___8Eg7M,
                    {
                      [sty.texttest___8Eg7MxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__nkw9K,
                    {
                      [sty.texttest__nkw9KxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>

              <div
                className={classNames(projectcss.all, sty.freeBox___7VlRq, {
                  [sty.freeBoxtest___7VlRqxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__ryKnk,
                    {
                      [sty.texttest__ryKnKxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__tIeYg,
                    {
                      [sty.texttest__tIeYgxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__dC401,
                    {
                      [sty.texttest__dC401XuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>

              <div
                className={classNames(projectcss.all, sty.freeBox__iLg9Y, {
                  [sty.freeBoxtest__iLg9YxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__yjmYb,
                    {
                      [sty.texttest__yjmYbxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Mode of Shipment"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__k6G56,
                    {
                      [sty.texttest__k6G56XuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {":"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__taJjn,
                    {
                      [sty.texttest__taJjNxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"N/A"}
                </div>
              </div>
            </div>
          ) : null}

          <p.Stack
            as={"div"}
            hasGap={true}
            className={classNames(projectcss.all, sty.freeBox__hYsmU, {
              [sty.freeBoxtest__hYsmUxuUp0]: hasVariant(
                variants,
                "test",
                "test"
              )
            })}
          >
            <div
              className={classNames(
                projectcss.all,
                projectcss.__wab_text,
                sty.text___3QoiB,
                {
                  [sty.texttest___3QoiBxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                }
              )}
            >
              {"Order To: "}
            </div>

            <div
              className={classNames(
                projectcss.all,
                projectcss.__wab_text,
                sty.text__vxo1G,
                {
                  [sty.texttest__vxo1GxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                }
              )}
            >
              {"Ship To:"}
            </div>
          </p.Stack>

          {true ? (
            <p.Stack
              as={"div"}
              hasGap={true}
              className={classNames(projectcss.all, sty.freeBox__bvZcN, {
                [sty.freeBoxtest__bvZcNxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__dTlRl,
                  {
                    [sty.texttest__dTlRlxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"ABC Company, \nLocation"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__iWg4U,
                  {
                    [sty.texttest__iWg4UxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"ABC Company, \nLocation"}
              </div>
            </p.Stack>
          ) : null}

          <p.Stack
            as={"div"}
            hasGap={true}
            className={classNames(projectcss.all, sty.freeBox__ext9H, {
              [sty.freeBoxtest__ext9HxuUp0]: hasVariant(
                variants,
                "test",
                "test"
              )
            })}
          >
            <div
              className={classNames(projectcss.all, sty.freeBox__tUmc, {
                [sty.freeBoxtest__tUmcxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__gr2Bz
                )}
              >
                {"Contact:"}
              </div>

              {true ? (
                <p.Stack
                  as={"div"}
                  hasGap={true}
                  className={classNames(projectcss.all, sty.freeBox__aiKdK, {
                    [sty.freeBoxtest__aiKdKxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  })}
                >
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__jZGfS,
                      {
                        [sty.texttest__jZGfSxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"Name:"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__ebobm,
                      {
                        [sty.texttest__ebobmxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"1234567890"}
                  </div>
                </p.Stack>
              ) : null}
            </div>

            <div
              className={classNames(projectcss.all, sty.freeBox__nmJ0, {
                [sty.freeBoxtest__nmJ0XuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text___58KzC,
                  {
                    [sty.texttest___58KzCxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Date:"}
              </div>

              {true ? (
                <p.Stack
                  as={"div"}
                  hasGap={true}
                  className={classNames(projectcss.all, sty.freeBox__hRpbe, {
                    [sty.freeBoxtest__hRpbexuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  })}
                >
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__z2Ixv,
                      {
                        [sty.texttest__z2IxvxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"1234567890"}
                  </div>
                </p.Stack>
              ) : null}
            </div>
          </p.Stack>

          <div
            className={classNames(projectcss.all, sty.freeBox__rczk3, {
              [sty.freeBoxtest__rczk3XuUp0]: hasVariant(
                variants,
                "test",
                "test"
              )
            })}
          >
            <div
              className={classNames(projectcss.all, sty.freeBox__fOoCi, {
                [sty.freeBoxtest__fOoCixuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__v3G,
                  {
                    [sty.texttest__v3GXuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"#"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__t2GL5,
                  {
                    [sty.texttest__t2GL5XuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Product ID"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__tEi7G,
                  {
                    [sty.texttest__tEi7GxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Product"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__fIjuW,
                  {
                    [sty.texttest__fIjuWxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Quantity"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__uMbP5,
                  {
                    [sty.texttest__uMbP5XuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Unit Price"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__nbm4U,
                  {
                    [sty.texttest__nbm4UxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Total"}
              </div>
            </div>

            <div className={classNames(projectcss.all, sty.freeBox__xoFrp)}>
              <div
                className={classNames(projectcss.all, sty.freeBox__qouMn, {
                  [sty.freeBoxtest__qouMNxuUp0]: hasVariant(
                    variants,
                    "test",
                    "test"
                  )
                })}
              >
                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text___3PZgD,
                    {
                      [sty.texttest___3PZgDxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"#"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__smgf6,
                    {
                      [sty.texttest__smgf6XuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Product ID"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__foePu,
                    {
                      [sty.texttest__foePuxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Product"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__mnDmh,
                    {
                      [sty.texttest__mnDmHxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Quantity"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__dw7AL,
                    {
                      [sty.texttest__dw7ALxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Unit Price"}
                </div>

                <div
                  className={classNames(
                    projectcss.all,
                    projectcss.__wab_text,
                    sty.text__z24Ao,
                    {
                      [sty.texttest__z24AOxuUp0]: hasVariant(
                        variants,
                        "test",
                        "test"
                      )
                    }
                  )}
                >
                  {"Total"}
                </div>
              </div>

              {(hasVariant(variants, "test", "test") ? true : true) ? (
                <div
                  className={classNames(projectcss.all, sty.freeBox___5BxoO, {
                    [sty.freeBoxtest___5BxoOxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  })}
                >
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__iQxjF,
                      {
                        [sty.texttest__iQxjFxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"#"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__qO1Yp,
                      {
                        [sty.texttest__qO1YPxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"Product ID"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__j5HTa,
                      {
                        [sty.texttest__j5HTAxuUp0]: hasVariant(
                          variants,
                          "test",
                          "test"
                        )
                      }
                    )}
                  >
                    {"Product"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__tEpuk
                    )}
                  >
                    {"Quantity"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text___2Ebi9
                    )}
                  >
                    {"Unit Price"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__uTlCy
                    )}
                  >
                    {"Total"}
                  </div>
                </div>
              ) : null}
              {(hasVariant(variants, "test", "test") ? true : true) ? (
                <div
                  className={classNames(projectcss.all, sty.freeBox__mQhRn, {
                    [sty.freeBoxtest__mQhRnxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  })}
                >
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__ga2Wt
                    )}
                  >
                    {"#"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__l1OFp
                    )}
                  >
                    {"Product ID"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text___9VV6R
                    )}
                  >
                    {"Product"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__gvfm6
                    )}
                  >
                    {"Quantity"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__z8Ikx
                    )}
                  >
                    {"Unit Price"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text___4CgCz
                    )}
                  >
                    {"Total"}
                  </div>
                </div>
              ) : null}
              {(hasVariant(variants, "test", "test") ? true : true) ? (
                <div
                  className={classNames(projectcss.all, sty.freeBox__w3Kq, {
                    [sty.freeBoxtest__w3KQxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  })}
                >
                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__awu5F
                    )}
                  >
                    {"#"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__g0QnA
                    )}
                  >
                    {"Product ID"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__cynjp
                    )}
                  >
                    {"Product"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__gW86
                    )}
                  >
                    {"Quantity"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text__hx3Sp
                    )}
                  >
                    {"Unit Price"}
                  </div>

                  <div
                    className={classNames(
                      projectcss.all,
                      projectcss.__wab_text,
                      sty.text___2Yk6Z
                    )}
                  >
                    {"Total"}
                  </div>
                </div>
              ) : null}
            </div>
          </div>

          <p.Stack
            as={"div"}
            hasGap={true}
            className={classNames(projectcss.all, sty.freeBox__ethva, {
              [sty.freeBoxtest__ethvaxuUp0]: hasVariant(
                variants,
                "test",
                "test"
              )
            })}
          >
            <p.Stack
              as={"div"}
              hasGap={true}
              className={classNames(projectcss.all, sty.freeBox___2LbLl, {
                [sty.freeBoxtest___2LbLlxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__mDnO,
                  {
                    [sty.texttest__mDnOxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Sub Total"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__jfbaE,
                  {
                    [sty.texttest__jfbaExuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Total"}
              </div>
            </p.Stack>

            <p.Stack
              as={"div"}
              hasGap={true}
              className={classNames(projectcss.all, sty.freeBox__yFmj0, {
                [sty.freeBoxtest__yFmj0XuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__joud
                )}
              >
                {"Tax"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__mRp5Y,
                  {
                    [sty.texttest__mRp5YxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Total"}
              </div>
            </p.Stack>

            <p.Stack
              as={"div"}
              hasGap={true}
              className={classNames(projectcss.all, sty.freeBox__btP0O, {
                [sty.freeBoxtest__btP0OxuUp0]: hasVariant(
                  variants,
                  "test",
                  "test"
                )
              })}
            >
              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__cVyyk,
                  {
                    [sty.texttest__cVyyKxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Total"}
              </div>

              <div
                className={classNames(
                  projectcss.all,
                  projectcss.__wab_text,
                  sty.text__cA3Yp,
                  {
                    [sty.texttest__cA3YPxuUp0]: hasVariant(
                      variants,
                      "test",
                      "test"
                    )
                  }
                )}
              >
                {"Total"}
              </div>
            </p.Stack>
          </p.Stack>

          {true ? (
            <ButtonB
              data-plasmic-name={"formbutton"}
              data-plasmic-override={overrides.formbutton}
              className={classNames("__wab_instance", sty.formbutton, {
                [sty.formbuttontest]: hasVariant(variants, "test", "test")
              })}
              connectmm={true}
            />
          ) : null}
        </p.Stack>
      </div>
    </div>
  );
}

const PlasmicDescendants = {
  root: ["root", "applicationForm", "formbutton"],
  applicationForm: ["applicationForm", "formbutton"],
  formbutton: ["formbutton"]
};

function makeNodeComponent(nodeName) {
  const func = function (props) {
    const { variants, args, overrides } = deriveRenderOpts(props, {
      name: nodeName,
      descendantNames: [...PlasmicDescendants[nodeName]],
      internalArgPropNames: PlasmicSalesordergenerationOld__ArgProps,
      internalVariantPropNames: PlasmicSalesordergenerationOld__VariantProps
    });

    return PlasmicSalesordergenerationOld__RenderFunc({
      variants,
      args,
      overrides,
      forNode: nodeName
    });
  };
  if (nodeName === "root") {
    func.displayName = "PlasmicSalesordergenerationOld";
  } else {
    func.displayName = `PlasmicSalesordergenerationOld.${nodeName}`;
  }
  return func;
}

export const PlasmicSalesordergenerationOld = Object.assign(
  // Top-level PlasmicSalesordergenerationOld renders the root element
  makeNodeComponent("root"),
  {
    // Helper components rendering sub-elements
    applicationForm: makeNodeComponent("applicationForm"),
    formbutton: makeNodeComponent("formbutton"),
    // Metadata about props expected for PlasmicSalesordergenerationOld
    internalVariantProps: PlasmicSalesordergenerationOld__VariantProps,
    internalArgProps: PlasmicSalesordergenerationOld__ArgProps
  }
);

export default PlasmicSalesordergenerationOld;
/* prettier-ignore-end */
