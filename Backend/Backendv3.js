...
const idAssetMinter = require("./ABI/IdAssetMinterABI");
const idAssetMinterAbi = JSON.parse(idAssetMinter);
const logisticsAssetMinter = require("./ABI/logisticsAssetMinterABI")
...
async function addAsset(JSdoc) {
    try {
        ...
        const coll = database.collection("idApplication");
    ...
    updateIdCard(req.body).then(function (response, error) {
        if (response) {
            if (response === 'ID Card does not exist') {
                ...
                res.status(200).json( {Status: "ID Card Successfully Updated"})
            }
        } else {
...
async function updateIdCard(JSdoc) {
    try {
        ...
        const Cursor = await coll.findOneAndUpdate(
            ...
            { $set: { "docno": JSdoc.docno, "imgUrl": JSdoc.imgUrl } }
        )
    ...
    addVerifyToken(req.body).then(function (response, error) {
        if (response) {
            if (response === 'ID Card does not exist') {
                ...
                res.status(200).json({ Status: "Token added to ID Card Successfully" })
            }
        } else {
            res.status(500).json({ "Error": error })
        }
...
app.post("/addNewId", function (req, res) {
    ...
    addNewId(req.body).then(function (response, error) {
        if (response) {
            console.log(response)
            res.status(200).json({ Status: "New ID Successfully Added" });
        } else {
            res.status(500).json({ "Error": error })
        }
    })
...
async function getIdCard(JSdoc) {
    try {
        ...
        const database = client.db(dbName);
        const coll = database.collection("idcard");
	const Cursor = await coll.find({ "docno": JSdoc.docno }).project({ _id: 0 })
        ...
        //const Cursor = await coll.findOne({ "docno": JSdoc.docno }).project({ _id: 0 })
        //if (Cursor) {
        //    return Cursor;
        //} else {
        //    return 'No Record Found';
...
app.get("/getIdCardVerify", function (req, res) {
    ...
    console.log("Reached here", req.body);
    getIdCardVerify(param).then(function (response, error) {
        if (response) {
            console.log(response)
            if (response === 'No Record Found') {
                res.status(404).json("No Record Found")
...
async function getIdCardVerify(JSdoc) {
    try {
        ...
        const database = client.db(dbName);
        const coll = database.collection("idcard");
        const Cursor = await coll.findOne({ "token": JSdoc.token });
        if (Cursor) {
            return Cursor;
        } else {
            return 'No Record Found';
